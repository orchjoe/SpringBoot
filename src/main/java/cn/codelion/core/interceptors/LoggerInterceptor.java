...
public class LoggerInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse httpServletResponse, Object o) throws Exception {
		...
		MapUtil.requestParamToMap(request.getParameterMap());
		return true;// 继续流程
	}
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception e) throws Exception {
		...
		try {
			if (consumeTime > 5000) {
				logger.error("用户:{},耗时:{}毫秒,URI:{},最大内存:{}m,已分配内存:{}m,已分配内存中的剩余空间:{}m,最大可用内存:{}m", consumeTime, uri, convertBToMB(Runtime.getRuntime().maxMemory()),
				...
				logger.debug("用户:{},耗时:{}毫秒,URI:{},最大内存:{}m,已分配内存:{}m,已分配内存中的剩余空间:{}m,最大可用内存:{}m", consumeTime, uri, convertBToMB(Runtime.getRuntime().maxMemory()),
						convertBToMB(Runtime.getRuntime().totalMemory()), convertBToMB(Runtime.getRuntime().freeMemory()),
						convertBToMB(Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory() + Runtime.getRuntime().freeMemory()));
			}
		} catch (Exception e2) {
